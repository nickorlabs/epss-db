...
mkdir -p epss-data
cd epss-data
EPSS_DATA_PATH="$BASEPATH/epss-data"
...
### from: wget https://epss.empiricalsecurity.com/epss_scores-2021-04-14.csv.gz
### to:   wget https://epss.empiricalsecurity.com/epss_scores-2021-08-31.csv.gz
cd $BASEPATH
mkdir 1st
cd 1st
...
do
        ...
        wget -q https://epss.empiricalsecurity.com/epss_scores-$CURRENTDATE.csv.gz
...
### from: wget https://epss.empiricalsecurity.com/epss_scores-2021-09-01.csv.gz
### to  : wget https://epss.empiricalsecurity.com/epss_scores-2022-02-03.csv.gz
cd $BASEPATH
mkdir 2nd
cd 2nd
...
### from: wget https://epss.empiricalsecurity.com/epss_scores-2022-02-04.csv.gz
...
mysql --defaults-extra-file="$BASEPATH/my.cnf" -u root epssdb -e "create index idx_cve on epssdb(cve);"
echo "  Create index for epss(score)."
mysql --defaults-extra-file="$BASEPATH/my.cnf" -u root epssdb -e "create index idx_epss on epssdb(epss);"
echo "  Create index for percentile."
mysql --defaults-extra-file="$BASEPATH/my.cnf" -u root epssdb -e "create index idx_percentile on epssdb(percentile);"
echo "  Create index for date."
mysql --defaults-extra-file="$BASEPATH/my.cnf" -u root epssdb -e "create index idx_date on epssdb(date);"
