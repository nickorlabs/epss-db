...
def main():
    parser = argparse.ArgumentParser(description="Fetch and translate CERT-FR /avis/ advisories.")
    ...
    while True:
        ...
        try:
            ...
            break
        soup = BeautifulSoup(html, "html.parser")
        articles = soup.select("article.item.cert-avis")  # Corrected selector for Avis based on actual HTML
        for art in articles:
            # Extract detail URL
            a = art.select_one("div.item-title a")  # Corrected selector for link within Avis article
            if not a:
                continue
